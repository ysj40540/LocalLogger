...
public class LogToFileUtils {
    ...
    private static final String LOG_DIR_NAME = "YorkLog";
        ...
        if (null == sContext || null == instance || null == sLogFile || !sLogFile.exists()) {
            ...
            logService.addExecuteTask(new Runnable() {
                ...
                public void run() {
                    ...
                    if (sLogFile != null) {
                        YLog.logInfo("LogFilePath is: " + sLogFile.getPath(), false);
                        // 获取当前日志文件大小
                        long logFileSize = getLogFileSize(sLogFile);
                        // 若日志文件超出了预设大小，则重置日志文件
                        if (LOG_MAX_SIZE < logFileSize) {
                            YLog.logInfo("init reset log file", false);
            ...
            YLog.logDebug("LogToFileUtils has been init ...", false);
        }
    }
    ...
    private long readSDCardSpace() {
        ...
        YLog.logDebug("sd卡存储空间:" + String.valueOf(sdCardSize) + "kb", false);
        return sdCardSize;
    }
    ...
    private long readSystemSpace() {
        ...
        YLog.logDebug("内部存储空间:" + String.valueOf(systemSpaceSize) + "kb", false);
    ...
    public void resetLogFile() {
        YLog.logDebug("Reset Log File ... ", false);

        // 创建log.csv，若存在则删除
        if (!sLogFile.getParentFile().exists()) {
            YLog.logDebug("Reset Log make File dir ... ", false);
    ...
    public void deleteLogFile() {
        ...
        File logFile = new File(sLogFile.getParent() + "/logs.csv");
        if (logFile.exists()) {
            YLog.logDebug("delete Log File ... ", false);
            logFile.delete();
        }
    }
    ...
    public void deleteLogFileDir() {
        ...
        if (dir.exists()) {
            YLog.logDebug("delete Log FileDir ... ", false);
            dir.delete();
        }
    }
    ...
    public void createNewFile(File logFile) {
        try {
            ...
            YLog.logError("Create log file failure !!! " + e.toString(), false);
        }
    }
